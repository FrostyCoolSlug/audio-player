...
use libpulse_binding::mainloop::signal::MainloopSignals;
...
impl PulseAudioConfiguration {
    fn new() -> Self {
        ...
        proplist.set_str(pulse::proplist::properties::APPLICATION_NAME, &app_name).unwrap();

        let main_loop = Rc::new(RefCell::new(Mainloop::new().expect("Failed to create MainLoop")));
        let context = Rc::new(RefCell::new(Context::new_with_proplist(main_loop.borrow().deref(), &app_name, &proplist).expect("Unable to create context"),));
